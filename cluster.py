...
def initSim():
    ...
    for i in range(num_community): #initiates cluster agents
        ...
        canvas.itemconfig(circles[i], fill="#FAA") #colors community agents red
        if i < num_apps: #agents with apps are magenta
            canvas.itemconfig(circles[i], fill="magenta")

    for i in range(num_community, num): #initializes the rest of the "normal" agents
        circles.append(canvas.create_oval(0, 0, rad, rad, fill="white", tags=num))
        text.append(canvas.create_text(0, 0, fill="black", font="Times 15 italic bold", text=str(i + 1)))
        velLines.append(canvas.create_line(0, 0, 10, 10, fill="red"))
        gvLines.append(canvas.create_line(0, 0, 10, 10, fill="green"))
        ...
        if hallway == 1: #from variables txt, if hallway, will run with 0.2 degrees radian freedom of movement
            ang = rnd.uniform(np.pi-0.2,np.pi+0.2)
        elif hallway == 0:
            ang = rnd.uniform(0, 2 * np.pi)
        v[i][0] = cos(ang) * velocity #increases velocity if specified
    ...
    for i in range(num):
        ...
        for j in range(num):
            ...
            if l2 < s2:
                nbr[i].append(j)
                nd[i].append(sqrt(l2))
                if [j, i] not in contacts_close: #count all close_contacts
                    contacts_close.append([i, j])
                    total_interactions += 1
                    if j < num_apps and i < num_apps: #count contacts with app
                        num_app_contacts += 1
...
def update(dt):
    global c, angs_arr, frac_of_circ, velocity
...
initSim()
maxIttr = 50000 #maximum number of iterations to run
